...
export function getRecommendedMap(stocks: Array<TMarketEntry>): Map<string, TGroupMarketEntry> {
...
// This function will group the recommendation
export function getSectorMap(stocks: Array<TMarketEntry>): Map<string, TGroupMarketEntry> {
...
export function processPositionData(obj: any, ltpMap: Map<string, number>): TPosition {
  //dlog.obj(obj)
  ...
  for (var x of obj) {
    ...
    let ltp = ltpMap.get(symbol.toUpperCase());
    //dlog.d(x);
    if (!ltp) {
      continue;
    }
